...
export const BankingProvider = ({ children }) => {
  ...
  const getAdminStats = async () => {
    try {
      ...
      return { success: false, message: err.response?.data?.message || 'Failed to fetch stats' }
    }
  }
  ...
  const searchUsers = async (q) => {
    try {
      ...
      return { success: false, message: err.response?.data?.message || 'Failed to search users' }
    }
  }

  const getAdminTransactions = async (params = {}) => {
    try {
      const response = await api.get('/admin/transactions/list', { params })
      return { success: true, data: response.data.data }
    } catch (err) {
      return { success: false, message: err.response?.data?.message || 'Failed to fetch transactions' }
    }
  }
  ...
  const updateUserStatus = async (id, status) => {
    try {
      ...
      return { success: false, message: err.response?.data?.message || 'Failed to update user status' }
    }
  }

  const createAdminTransaction = async (payload) => {
    try {
      const response = await api.post('/admin/create-transaction', payload)
      return { success: true, data: response.data.data, message: response.data.message, newBalance: response.data.newBalance }
    } catch (err) {
  ...
  const createAdminUser = async (payload) => {
    try {
      ...
      return { success: false, message: err.response?.data?.message || 'Failed to create user account' }
    }
  }
